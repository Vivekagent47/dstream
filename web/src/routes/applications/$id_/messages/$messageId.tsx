...
import { api, qk, type MessageDeliveryAttempt } from '#/lib/api'
...
const messageQuery = (id: string, messageId: string) =>
  queryOptions({
    queryKey: qk.message(id, messageId),
    queryFn: () => api.getMessage(id, messageId),
  })

export const Route = createFileRoute('/applications/$id_/messages/$messageId')({
  // Client-only prefetch — SSR can't forward the session cookie.
  loader: ({ context, params }) =>
    typeof window === 'undefined'
      ? undefined
      : context.queryClient.ensureQueryData(messageQuery(params.id, params.messageId)),
...
function MessageDetail() {
  const { id, messageId } = Route.useParams()
  const { data: msg } = useQuery(messageQuery(id, messageId))
  ...
  return (
    <div className="flex flex-1 flex-col">
      <PageHeader
        title={
          <span className="flex min-w-0 items-center gap-1.5">
            ...
            <span className="truncate font-mono text-sm">{msg.event_type}</span>
          </span>
        }
      />
      ...
      <div className="flex-1 space-y-8 overflow-y-auto px-6 py-4">
        ...
        <Activity appId={id} messageId={messageId} />
      </div>
    </div>
  )
}

function Activity({ appId, messageId }: { appId: string; messageId: string }) {
  const qc = useQueryClient()
  const [expanded, setExpanded] = useState<Set<string>>(new Set())

  const { data: deliveries, error: delErr } = useQuery({
    queryKey: qk.messageDeliveries(appId, messageId),
    queryFn: () => api.listMessageDeliveries(appId, messageId),
    refetchInterval: 5000,
  })
  const { data: attempts, error: attErr } = useQuery({
    queryKey: qk.messageAttempts(appId, messageId),
    queryFn: () => api.listMessageAttempts(appId, messageId),
    refetchInterval: 5000,
  })
  ...
  const replay = useMutation({
    mutationFn: (endpointId: string) => api.replayDelivery(appId, messageId, endpointId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: qk.messageDeliveries(appId, messageId) })
      qc.invalidateQueries({ queryKey: qk.messageAttempts(appId, messageId) })
